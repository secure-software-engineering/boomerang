...
public class BackwardFlowFunctions extends AbstractFlowFunctions implements
        ...
        if (leftOp instanceof Local && source.baseMatches(leftOp)) {
          if (isAllocationSite(rightOp)) {
            if (source.getFieldCount() > 0
                && !source.getFirstField().getField().equals(AliasFinder.ARRAY_FIELD)) {
              return Collections.emptySet();
            }
            ...
            if (op instanceof Local) {
              if (typeCompatible(((Local) op).getType(), source.getBaseType())) {
                return Collections.singleton(source.deriveWithNewLocal((Local) op,
                    source.getBaseType()));
              } else
                return Collections.emptySet();
            }
            ...
            if (typeCompatible(((Local) rightOp).getType(), source.getBaseType())) {
              return Collections.singleton(source.deriveWithNewLocal((Local) rightOp,
                  source.getBaseType()));
            } else {
              return Collections.emptySet();
            }
            ...
            if (fr.getBase() instanceof Local) {
              ...
              WrappedSootField newFirstField =
                  new WrappedSootField(fr.getField(), source.getBaseType(), curr);
            ...
            Set<AccessGraph> out = new HashSet<>();
            AccessGraph prependField =
                source.prependField(new WrappedSootField(AliasFinder.ARRAY_FIELD,
                    source.getBaseType(), curr));
            ...
            AccessGraph prependField =
                source.prependField(new WrappedSootField(fr.getField(), source.getBaseType(), curr));
          ...
          if (source.baseAndFirstFieldMatches(base, field)) {
            ...
            if (rightOp instanceof Local) {
              AccessGraph newAp =
                  source.deriveWithNewLocal((Local) rightOp, source.getFirstField().getType());
              return newAp.popFirstField();
            }
          }
          ...
          if (source.isStatic() && source.firstFieldMatches(sfr.getField())) {
            if (rightOp instanceof Local) {
              ...
              for (AccessGraph a : newAp) {
                out.add(a.deriveWithNewLocal((Local) rightOp, source.getFirstField().getType()));
              }
              return out;
            }
          }
  ...
  @Override
  public FlowFunction<AccessGraph> getReturnFlowFunction(final IPathEdge<Unit, AccessGraph> edge,
      final Unit callSite, final SootMethod callee, final Unit returnSite) {
              ...
              if (source.getBase().equivTo(paramLocals[i])) {
                ...
                if (arg instanceof Local) {
                  if (typeCompatible(((Local) arg).getType(), source.getBaseType())) {
                    AccessGraph ap =
                        derivedSource.deriveWithNewLocal((Local) arg, source.getBaseType());
                    out.add(ap);
                  }
                }
              }
  ...
  private void allocationSiteReached(IPathEdge<Unit, AccessGraph> pe, AssignStmt as, Value val) {
    ...
    if (leftOp instanceof Local) {
      if (factAtTarget.getFieldCount() > 0) {
        if (factAtTarget.getFirstField().getField().equals(AliasFinder.ARRAY_FIELD)) {
        ...
        if (!Scene.v().getOrMakeFastHierarchy()
            .canStoreType(val.getType(), context.getSubQuery().getType())){
          return;
        }
      }
