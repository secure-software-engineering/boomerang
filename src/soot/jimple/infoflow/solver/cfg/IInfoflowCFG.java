...
public interface IInfoflowCFG extends BiDiInterproceduralCFG<Unit, SootMethod> {
  ...
  /**
   * Abstraction of a postdominator. This is normally a unit. In cases in which a statement does not
   * have a postdominator, we record the statement's containing method and say that the
   * postdominator is reached when the method is left. This class MUST be immutable.
   *
   * @author Steven Arzt
   */
  public static final class UnitContainer {
    ...
    @Override
    public int hashCode() {
      return 31 * (unit == null ? 0 : unit.hashCode()) + 31
  ...
  /**
   * Gets the postdominator of the given unit. If this unit is a conditional, the postdominator is
   * the join point behind both branches of the conditional.
   * 
  ...
  /**
   * Checks whether the given static field is read inside the given method or one of its transitive
   * callees.
   * 
   * @param method The method to check
   * @param variable The static field to check
   * @return True if the given static field is read inside the given method, otherwise false
   */
  public boolean isStaticFieldRead(SootMethod method, SootField variable);

  /**
   * Checks whether the given static field is used (read or written) inside the given method or one
   * of its transitive callees.
   * 
   * @param method The method to check
   * @param variable The static field to check
   * @return True if the given static field is used inside the given method, otherwise false
   */
   ...
   * 
   * @param method The method to check
   * @return True if the given method or one of its transitive callees has side effects, otherwise
   *         false
   */
  ...
  public boolean containsAllocSiteOfType(SootMethod methodOf, Type type);

  public boolean accessesField(SootMethod callee, SootField firstField);

  public boolean writesToField(SootMethod callee, SootField firstField);
